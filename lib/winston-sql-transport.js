...
class SQLTransport extends Transport {
   ...
   * @param {boolean} [options.keepLocalTime=false] - Boolean flag indicating whether to
   * keepLocalTime for timestamp. When True, the server's timezone local time
   * is stored to SQL Server timestamp column. When False, the server's time is
   * converted to UTC and stored to SQL Server timestamp column as UTC.
   */
  constructor(options = {}) {
    super();
    ...
    this.keepLocalTime = options.keepLocalTime || false;
  }
  ...
  log(...args) {
    ...
    const { client, tableName, keepLocalTime } = this;

    return client
      .insert({
        timestamp: moment().utc(keepLocalTime).startOf('second').toDate(),
        level,
        message,
  ...
  query(...args) {
    ...
    if (options.from && options.until) {
      query = query.whereBetween('timestamp', [
        moment(options.from).utc(this.keepLocalTime).toDate(),
        moment(options.until).utc(this.keepLocalTime).toDate()
      ]);
    }
